...
def get_train_val_loaders(train_dir,
    ...
    # train_dir_temp = copy_class_images_to_temp_dir(trainset.csv_df, opts.dir_working, opts.train_dir, 'train')
    # trainset.root_dir = train_dir_temp
    # Validation dataset
    ...
    # val_dir_temp = copy_class_images_to_temp_dir(valset.csv_df, opts.dir_working, opts.val_dir, 'val')
    # valset.root_dir = val_dir_temp
    ...
    return trainset_loader, valset_loader#, train_dir_temp, val_dir_temp

def copy_class_images_to_temp_dir(df, temp_dir, dir_source, train_or_val):
    os.makedirs(os.path.join(temp_dir, train_or_val), exist_ok=True)
    for filename in df.iloc[:, 0]:
        src_path = os.path.join(dir_source, filename)
        dst_path = os.path.join(temp_dir, train_or_val, filename)
        shutil.copy(src_path, dst_path)
    return os.path.join(temp_dir, train_or_val)
...
class CSVDataset(torch.utils.data.Dataset):
        ...
        if landmark is None:
            ...
            for filename in listfiles:
                if filename.endswith('.csv'):
                    ...
                    if landmark in filename_parts:
                        if train_or_val == 'train':
                            if 'train' in filename_parts:
                                csv_filename = filename
                                break
                        elif train_or_val == 'val':
                            if 'test' in filename_parts:
                                csv_filename = filename
                                break
    ...
    def __getitem__(self, idx):
        ...
        if self.there_is_gt:
            # print(f'idx = {idx}')
            # print(f'filename = {self.csv_df.loc[idx].filename}')
