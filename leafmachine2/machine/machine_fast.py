...
from leafmachine2.machine.general_utils import check_for_subdirs, get_datetime, load_config_file, load_config_file_testing, report_config, split_into_batches, split_into_batches_spoof, save_config_file, subset_dir_images, crop_detections_from_images, crop_detections_from_images_SpecimenCrop
...
def machine(cfg_file_path, dir_home, cfg_test, progress_report=None):
    ...
    for dir_ind, dir_in in enumerate(dirs_list):
        ...
        Project, n_batches, m = split_into_batches_spoof(Project, logger, cfg)
        if progress_report:
        ...
        for batch in range(n_batches):
            ...
            if cfg['leafmachine']['do']['run_leaf_processing']:
                ...
                logger.name = f'[COMPLETE]'

                t_batch = f"[elapsed time] {round(t_batch_end - t_batch_start)} seconds ({round((t_batch_end - t_batch_start)/60)} minutes)"
                logger.info(t_batch)
                time_report[f't_batch'] = t_batch
