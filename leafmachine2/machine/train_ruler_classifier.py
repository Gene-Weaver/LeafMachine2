...
class TrainOptions:
    ...
    def __post_init__(self) -> None:
        ...
        if self.cfg['leafmachine']['ruler_train']['split_train_dir_automatically'] is not None:
            self.split_train_dir_automatically = self.cfg['leafmachine']['ruler_train']['split_train_dir_automatically']

        if self.cfg['leafmachine']['ruler_train']['split_train_dir_consistently'] is not None:
            self.split_train_dir_consistently = self.cfg['leafmachine']['ruler_train']['split_train_dir_consistently']

        if self.cfg['leafmachine']['ruler_train']['save_all_checkpoints'] is not None:
            self.save_all_checkpoints = self.cfg['leafmachine']['ruler_train']['save_all_checkpoints']

        if self.cfg['leafmachine']['ruler_train']['n_machines'] is not None:
            self.n_machines = int(self.cfg['leafmachine']['ruler_train']['n_machines'])
        
        if self.cfg['leafmachine']['ruler_train']['n_gpus'] is not None:
            self.n_gpus = int(self.cfg['leafmachine']['ruler_train']['n_gpus'])

        if self.cfg['leafmachine']['ruler_train']['default_timeout_minutes'] is not None:
            self.default_timeout_minutes = int(self.cfg['leafmachine']['ruler_train']['default_timeout_minutes'])
        
        if self.cfg['leafmachine']['ruler_train']['img_size'] is not None:
            self.img_size = int(self.cfg['leafmachine']['ruler_train']['img_size'])

        if self.cfg['leafmachine']['ruler_train']['use_cuda'] is not None:
            self.use_cuda = self.cfg['leafmachine']['ruler_train']['use_cuda']

        if self.cfg['leafmachine']['ruler_train']['batch_size'] is not None:
            self.batch_size = int(self.cfg['leafmachine']['ruler_train']['batch_size'])

        if self.cfg['leafmachine']['ruler_train']['learning_rate'] is not None:
            self.learning_rate = float(self.cfg['leafmachine']['ruler_train']['learning_rate'])
            
        if self.cfg['leafmachine']['ruler_train']['n_epochs'] is not None:
            self.n_epochs = int(self.cfg['leafmachine']['ruler_train']['n_epochs'])

        if self.cfg['leafmachine']['ruler_train']['print_every'] is not None:
            self.print_every = int(self.cfg['leafmachine']['ruler_train']['print_every'])

        if self.cfg['leafmachine']['ruler_train']['num_classes'] is not None:
            self.num_classes = int(self.cfg['leafmachine']['ruler_train']['num_classes'])

        if self.cfg['leafmachine']['ruler_train']['seed_for_random_split'] is not None:
            self.num_classes = int(self.cfg['leafmachine']['ruler_train']['seed_for_random_split'])

        # If  model name is given, use it, else use default plus current time
        if self.cfg['leafmachine']['ruler_train']['model_name'] is not None:
            if not self.cfg['leafmachine']['ruler_train']['do_overwrite_model']:
                if self.cfg['leafmachine']['ruler_train']['model_name'] in os.listdir(self.path_to_model):
                    self.model_name = "__".join([self.cfg['leafmachine']['ruler_train']['model_name'], self.new_time])
                else:
                    self.model_name = self.cfg['leafmachine']['ruler_train']['model_name']
            else:
                self.model_name = self.cfg['leafmachine']['ruler_train']['model_name']
        else:
...
netPath = os.path.join(modelPath,'resnet.pt')
...
# plt.ioff()
'''
def parse_train_ruler_classifier():
    ...
    required_args.add_argument('--dir-train',
                                type=str,
                                required=True,
                                help='')
    required_args.add_argument('--dir-val',
                                type=str,
                                required=True,
                                help='')
    required_args.add_argument('--model-name',
                                ...
                                required=True,
                                help='')
    required_args.add_argument('--ruler-class-names',
                                type=str,
                                required=True,
                                help='')
    required_args.add_argument('--do-overwrite-model',
                                type=bool,
                                required=True,
                                help='')
    required_args.add_argument('--split-train_dir-automatically',
                                type=bool,
                                required=True,
                                help='')
    required_args.add_argument('--split-train-dir-consistently',
                                type=bool,
                                required=True,
                                help='')
    required_args.add_argument('--seed-for-random-split',
                                type=int,
                                required=True,
                                help='')
    required_args.add_argument('--img-size',
                                type=int,
                                required=True,
                                help='')
    required_args.add_argument('--use-cuda',
                                type=bool,
                                required=True,
                                help='')
    required_args.add_argument('--batch-size',
                                type=int,
                                required=True,
                                help='')
    required_args.add_argument('--learning-rate',
                                type=float,
                                required=True,
                                help='')
    required_args.add_argument('--n-epochs',
                                type=int,
                                required=True,
                                help='')
    required_args.add_argument('--print-every',
                                type=int,
                                required=True,
                                help='')
    required_args.add_argument('--num-classes',
                                type=int,
                                required=True,
                                help='')
