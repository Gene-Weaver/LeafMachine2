...
def count_psnr(cfg, epoch, valid_data='2018',setting='',flipped = False , thresh = 0.5):
    avg_psnr = 0
    qo = 0
    folder = os.path.join('vis', ''.join(['epoch', str(epoch)]))
    gt_folder = os.path.join(cfg.data_path, 'DIBCOSETS',valid_data,'gt_imgs')
    ...
    if not os.path.exists(os.path.join('vis', setting, 'epoch', str(epoch), '00_reconstr_', flip_status)):
        os.makedirs(os.path.join('vis', setting, 'epoch', str(epoch), '00_reconstr_', flip_status))


    for im in gt_imgs:


        gt_image = cv2.imread(os.path.join(gt_folder,im))
...
def count_psnr_both(cfg, epoch, valid_data='2018',setting='',thresh = 0.5):
    avg_psnr = 0
    qo = 0
    folder = os.path.join('vis', ''.join(['epoch', str(epoch)]))
    gt_folder = os.path.join(cfg.data_path, 'DIBCOSETS',valid_data,'gt_imgs')
