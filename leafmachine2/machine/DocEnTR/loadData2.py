...
class Read_data(D.Dataset):
    def __init__(self, cfg, file_label,set, augmentation=True , flipped = False):
        self.file_label = file_label
        self.set = set
        self.augmentation = augmentation
        self.flipped  = flipped

        self.baseDir = cfg.data_path
        self.split_size  = cfg.split_size
    ...
    def readImages(self, file_name):
        file_name = file_name
        url_deg = os.path.join(self.baseDir, self.set, file_name) #self.baseDir +'/'+ self.set+'/' + file_name
        url_gt = os.path.join(self.baseDir, ''.join([self.set, '_gt']), file_name)  #self.baseDir +'/'+ self.set+'_gt/'+file_name
...
def loadData_sets(cfg, flipped=False):
    
    data_tr = os.listdir(os.path.join(cfg.data_path,'train'))

    
    np.random.shuffle(data_tr)
    data_va = os.listdir(os.path.join(cfg.data_path, 'valid'))
    np.random.shuffle(data_va)
    data_te = os.listdir(os.path.join(cfg.data_path, 'test'))
    np.random.shuffle(data_te)
    
    data_train = Read_data(cfg, data_tr,'train', augmentation=True)
    data_valid = Read_data(cfg, data_va,'valid',augmentation=False,flipped = flipped)
    data_test = Read_data(cfg, data_te,'test',augmentation=False)
