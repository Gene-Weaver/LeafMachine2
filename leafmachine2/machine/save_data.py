...
class Data_Vault():
    ...
    def __init__(self, cfg, logger, filename, analysis, Dirs) -> None:
        ...
        # self.specimen_record = pd.DataFrame(analysis['GBIF_Record'])
        if 'GBIF_Record' in analysis:
            self.specimen_record = analysis['GBIF_Record']
        else:
            self.specimen_record = []

        if 'Detections_Archival_Components' in analysis:
            self.archival = analysis['Detections_Archival_Components']
        else:
            self.archival = []

        if 'Detections_Plant_Components' in analysis:
            self.plant = analysis['Detections_Plant_Components']
        else:
            self.plant = []

        if 'Ruler_Info' in analysis:
            self.ruler_info = analysis['Ruler_Info']
        else:
            self.ruler_info = []

        # if 'Ruler_Data' in analysis:
        #     self.ruler_data = analysis['Ruler_Data']
        # else:
        #     self.ruler_data = []

        try:
            self.seg_whole_leaf = analysis['Segmentation_Whole_Leaf']
        except:
            self.seg_whole_leaf = []

        try:
            self.seg_partial_leaf = analysis['Segmentation_Partial_Leaf']
        except:
        ...
        if len(ruler_info) == 0:
            ...
            for i in range(len(ruler_info)):
                ...
                if ruler_info_dict == []:
                    ...
                    ruler_location = ruler_image_name.split('__')[2]
        ...
        if len(ruler_info) == 0:
            ...
            for i in range(len(ruler_info)):
                ...
                if ruler_info_dict == []:
                    ...
                    ruler_class = ruler_info_dict.ruler_class
                    ruler_class_confidence = ruler_info_dict.ruler_class_percentage
                    ruler_location = ruler_info_dict.img_fname.split('__')[2]
                    ruler_location = [int(ruler_location.split('-')[0]),int(ruler_location.split('-')[1]),int(ruler_location.split('-')[2]),int(ruler_location.split('-')[3])]
