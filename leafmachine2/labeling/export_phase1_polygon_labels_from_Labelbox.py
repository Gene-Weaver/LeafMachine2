...
from segmentation.detectron2.detectron2.structures import BoxMode
...
def extract_polygon(image, polygon_pts):
    # Convert polygon points to a list of tuples
    pts = [(pt['x'], pt['y']) for pt in polygon_pts]
    ...
    # Extract the corresponding mask ROI
    mask_roi = mask[y:y+h, x:x+w]

    # Initialize min_height and min_width with dimensions of ROI and mask_roi
    min_height = min(mask_roi.shape[0], roi.shape[0])
    min_width = min(mask_roi.shape[1], roi.shape[1])

    # Ensure that the dimensions of mask_roi and roi match
    mask_roi = mask_roi[:min_height, :min_width]
    roi = roi[:min_height, :min_width]

    # Create a white image of the same size as the bounding box + 5 px padding
    padding = 5
    white_image = np.ones((h + 2 * padding, w + 2 * padding, 3), dtype=np.uint8) * 255

    # Extract the polygon area from the ROI
    extracted_polygon = np.zeros_like(roi)
    extracted_polygon[mask_roi == 1] = roi[mask_roi == 1]

    # Place the extracted polygon into the center of the white image
    white_image[padding:padding+min_height, padding:padding+min_width][mask_roi == 1] = extracted_polygon[mask_roi == 1]
            ...
            if annoType in opt.RESTRICT_ANNOTYPE:
                ...
                if dirState:
                    ...
                    if nImgs == 0: # Unstarted datasets
                        ...
                        path_clean_polygon_extracted_image = os.path.join(dir_seg,'extracted', 'clean_polygon')
                        path_complex_polygon_extracted_image = os.path.join(dir_seg,'extracted', 'complex_polygon')
                        validate_dir(path_clean_polygon_extracted_image)
                        validate_dir(path_complex_polygon_extracted_image)
                                ...
                                # elif (img['Reviews'] == []) and (opt.ONLY_REVIEWED):
                                    ...
                                    for proj in img['projects']:
                                        for label in img['projects'][proj]['labels'][0]['annotations']['objects']:
                                            # list
                                            if label['value'] == 'clean_polygon':
                                                polygon_pts = label['polygon']
                                                extracted_image = extract_polygon(img_color, polygon_pts)

                                                count_clean += 1
                                                fname_extracted = Path(external_id).with_name(f"{Path(external_id).stem}__clean_{count_clean}.jpg")
                                                extracted_image.save(os.path.join(path_clean_polygon_extracted_image, fname_extracted))


                                            if label['value'] == 'complex_polygon':
                                                polygon_pts = label['polygon']
                                                extracted_image = extract_polygon(img_color, polygon_pts)
                                                
                                                count_complex += 1
                                                fname_extracted = Path(external_id).with_name(f"{Path(external_id).stem}__complex_{count_complex}.jpg")
                                                extracted_image.save(os.path.join(path_complex_polygon_extracted_image, fname_extracted))
                                                

                                    if extracted_image:
                                        '''#TODO once we need to train, reactivate the polygon mask part'''
...
def export_segmentation_labels():
    ...
    path_cfg = os.path.join(dir_labeling,'config_export_phase1_polygon_labels_from_Labelbox.yaml')
