                    ...
                    if dirState:
                        ...
                        if nImgs == 0: # Unstarted datasets
                            ...
                            for img_data in tqdm(data, desc=f'{bcolors.BOLD}      Converting  >>>  {file}{bcolors.ENDC}',colour=cc,position=0):
                                ...
                                keypoint_mapper = KeypointMapping(trace_version = 'corners') # mid30_pet10 or 
                                Labels = Points(IMG_NAME=img_name,IMG_FILENAME=img_filename,VERSION = 'corners')
                                ...
                                for proj in img_data['projects']:
                                    for label in img_data['projects'][proj]['labels'][0]['annotations']['objects']:
                                        ...
                                        if label['value'] == 'corner':
                                            ...
                                            corners_unordered.append((pt_x,pt_y,2))

                                        if label['value'] == 'corner_infer':
                                            pt_x,pt_y = label['point'].values()
                                            pt_x,pt_y = convert_to_yolo_format(pt_x, pt_y, width, height)
                                            corners_unordered.append((pt_x,pt_y,1))
                                ...
                                if corners_unordered:
                                    ...
                                    im = overlay_trace_on_image_lines(im, corners_ordered, width, height)
                                    
                                    # Now map the ordered corners to the respective keypoints
                                    keypoint_mapper = KeypointMapping(trace_version='corners')
                                    for i, keypoint in enumerate(['top_left', 'top_right', 'bottom_left', 'bottom_right']):
                                        index = keypoint_mapper.get_index(keypoint)
                                        pt_x, pt_y, visibility = corners_ordered[i]
                                        Labels.KEYPOINTS[index] = [(pt_x, pt_y, visibility)]
                                        if visibility == 1:
                                            im = overlay_trace_on_image(im, [(pt_x, pt_y)], keypoint, width, height, style='hidden')
                                        elif visibility == 2:
                                            im = overlay_trace_on_image(im, [(pt_x, pt_y)], keypoint, width, height)
...
def export_points_labels():
    ...
    path_cfg = os.path.join(dir_labeling,'config_export_keypoints_corners_labels_from_Labelbox.yaml')
