...
def polygon_properties(polygon, out_label, seg_name_short, cfg, img_cropped):
    ...
    # Get approximate polygon to simplify rotation in fit_min_bbox()
    ...
    polygon_approx = np.array(cv2.approxPolyDP(polygon_contour, 0.010 * contour_perimeter, True)).reshape(-1, 2)
    # if polygon_approx.shape[0] < 10:
...
def fit_min_bbox(find_minimum_bounding_box, polygon, polygon_approx, c_x, c_y, tolerance=2):
    ...
    rect_orig = [(int(x), int(y)), (int(x + w), int(y)), (int(x + w), int(y + h)), (int(x), int(y + h))]
    ...
    if find_minimum_bounding_box:
        if abs(max_length - (2 * radius)) < tolerance:
            ...
            return rect_orig, polygon, angle, rotate_polygon_by_angle(rect, -angle, c_x, c_y), max_length, min_length
    else:
...
def rotate_polygon_by_angle(polygon, angle, c_x, c_y):
    ...
    rotated_polygon = [(int(x+c_x), int(y+c_y)) for x, y in rotated_polygon]
