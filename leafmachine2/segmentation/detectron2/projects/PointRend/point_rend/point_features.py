...
def get_uncertain_point_coords_on_grid(uncertainty_map, num_points):
    ...
    point_coords[:, :, 1] = h_step / 2.0 + torch.div(point_indices, W, rounding_mode='trunc').to(torch.float) * h_step # point_coords[:, :, 1] = h_step / 2.0 + (point_indices // W).to(torch.float) * h_step
